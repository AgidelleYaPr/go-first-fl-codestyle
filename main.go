...
func attack(name, class string) string {
	if class == "warrior" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", name, 5+randomize(3, 5))
	}

	if class == "mage" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", name, 5+randomize(5, 10))
	}

	if class == "healer" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", name, 5+randomize(-3, -1))
	}
...
func defence(name, class string) string {
	if class == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+randomize(5, 10))
	} else if class == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+randomize(-2, 2))
	} else if class == "healer" {
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+randomize(2, 5))
	}
	return "неизвестный класс персонажа"
